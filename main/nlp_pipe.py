...
class NLP_PIPE:
    ...
    def __init__(self):
        ...
        try:
            self.spacy = spacy.load('en_core_web_lg', enable=["fastcoref", "doc_cleaner"], 
                                    exclude=["ner"])
    ...
    def _extract_message_features(self, msg: MessageData, entity_threshold: float, res: Dict):
        ...
        entities = None
        with torch.no_grad():
            entities = self.gliner.predict_entities(text=clean_text, labels=self.gliner_labels)
    ...
    def _contextual_features(self, message_block: Tuple[str, List[str]], res: Dict):
        ...
        if not message_block or not str_version.strip():
            return


        doc = self.spacy(str_version)

        current_msg = list_version[-1]

        offset_start = str_version.rfind(current_msg)

        if offset_start == -1:
            offset_start = len(str_version) - len(current_msg)

        offset_end = offset_start + len(current_msg)

        curr_msg_span = doc.char_span(offset_start, offset_end)

        appositive_map = {}
        for token in curr_msg_span:
            if token.dep_ == "appos":
                head = token.head
        ...
        if doc._.coref_clusters:
            for cluster_spans in doc._.coref_clusters:
                if len(cluster_spans) < 2: continue
                ...
                for start, end in cluster_spans:
                    ...
                    if span_doc:
                        mentions_text.append(span_doc.text)
                        if start >= offset_start:
                            relevant_info = True
                
                if relevant_info and mentions_text:
                    main_span_doc = doc.char_span(cluster_spans[0][0], cluster_spans[0][1])
                    main_text = main_span_doc.text if main_span_doc else "Unknown"
        ...
        for i, entity in enumerate(high_conf_entities):
            ent_start = entity["span"][0] + offset_start
            ent_end = entity["span"][1] + offset_start
        ...
        for i, entity in enumerate(res["low_confidence_entities"]):
            ...
            for chunk_text, chunk_start, chunk_end in noun_chunks:
                if ent_start >= chunk_start and ent_end <= chunk_end:
                    ...
                    if chunk_text not in chunk_to_ent_indices:
                        chunk_to_ent_indices[chunk_text] = []
                    chunk_to_ent_indices[chunk_text].append(i)
                    break
        ...
        if chunk_texts:
            with torch.no_grad():
                batched_results = self.gliner.predict_entities(text=chunk_texts, labels=self.gliner_labels)
        ...
        for i, chunk_text in enumerate(chunk_texts):
            ...
            for new_ent in cleaned_entities_in_chunk:
                if new_ent['score'] >= 0.8:
                    global_start = new_ent['start'] + chunk_start
                    global_end = new_ent['end'] + chunk_start
                    
                    msg_relative_start = global_start - offset_start
                    msg_relative_end = global_end - offset_start

                    if msg_relative_start >= 0:
                        high_conf_entities.append({
                            "text": new_ent["text"],
                            "span": (msg_relative_start, msg_relative_end),
    ...
    def start_process(self, message_block: Tuple[str, List[str]], msg: MessageData,
        ...
        self._extract_message_features(msg=msg.message.strip(), entity_threshold=entity_threshold, res=res)
